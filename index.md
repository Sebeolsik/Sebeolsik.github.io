...
navbar: ["세벌식이란", "세벌식이 좋은 이유", "세벌식을 씁시다", "세벌식에 관한 오해와 사실들", "나에게 맞는 세벌식 찾기", "세벌식을 쓰는 방법"]
---

## 세벌식이란?
세벌식은 자음, 모음, 받침으로 나누어진 키보드 자판 배열을 뜻합니다.

현재 우리나라의 표준 자판(KS X 5002)은 자음과 모음이 각 한 벌씩 배치되어 있는 두벌식 자판입니다. 이에 비해 세벌식 자판은 자음, 모음, 받침이 각각 한 벌씩 배치되어 있습니다.
...
故 공병우 박사가 개발한 통칭 공세벌식 자판은 자음이 오른쪽, 모음이 중간, 받침이 왼쪽에 배열되어 있습니다. 한글의 한 음절은 자음으로 시작해서 모음이나 받침으로 끝나기 때문에 세벌식 자판으로 타자를 치면 한 음절을 칠 때의 흐름은 오른쪽에서 시작해서 왼쪽에서 끝나게 됩니다.

이 과정을 빠르게 반복하게 되면서 타자가 리드미컬하게 쳐지는, 일명 '리듬감'이 발생하게 됩니다.
...
세벌식을 지원하는 일부 입력기 소프트웨어에서는 '모아주기'라는 기능을 지원합니다. 이 기능을 사용하면 자음·모음·받침의 순서를 틀리게 치더라도 알아서 교정해 줍니다. 'ㅅ-ㅡ-ㅂ-ㅣ-ㄴ-ㄷ-ㅏ'의 순서로 치더라도 자동으로 '습니다'가 된다는 것이죠. 어떻게 그런 게 가능하냐고요?
...
1. **각 글쇠는 그에 맞는 손가락으로 누르도록 한다.** - 세벌식은 운지법을 익히지 않으면 그 가치가 대폭 감소합니다.
...
* 두벌식 자판 배열표 - [위키백과](https://ko.wikipedia.org/wiki/%EB%91%90%EB%B2%8C%EC%8B%9D_%EC%9E%90%ED%8C%90#/media/File:2set.png)
* 세벌식 3-90, 최종 배열표 - 한글문화원
